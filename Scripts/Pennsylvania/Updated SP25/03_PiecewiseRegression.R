...
csv_files <- list.files(path = "Data Management/Csvs/Hen_NSD_Data/", pattern = ".csv", full.names = TRUE)

# Define the model syntax for m3 (three intercepts)
model_m3 <- list(displacement ~ 1, ~ 1, ~ 1)
...
for (file in csv_files) {
  ...
  # Fit the model
  model_fit <- mcp(model = model_m3, data = df[, c("days_numeric", "displacement")],
